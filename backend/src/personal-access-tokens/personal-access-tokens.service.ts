...
} from "../auth/personal-access-token.util";
import type { CreatePersonalAccessTokenDto } from "./dto";
...
export class PersonalAccessTokensService {
  ...
  async create(userId: string, dto: CreatePersonalAccessTokenDto) {
    const permission = dto.permission as PersonalAccessTokenPermission;
    ...
    const { raw, hash, prefix } = generatePersonalAccessToken();
    ...
    const record = await this.prisma.personalAccessToken.create({
      data: {
        ...
        tokenHash: hash,
        prefix,
        scope: dto.scope as PersonalAccessTokenScope,
        permission,
        workspaceId,
        createdById: userId,
        expiresAt,
  ...
  async list(userId: string): Promise<PersonalAccessTokenView[]> {
  ...
  async revoke(userId: string, id: string): Promise<PersonalAccessTokenView> {
    const record = await this.prisma.personalAccessToken.findUnique({ where: { id } });
  ...
  private toView(t: PersonalAccessToken): PersonalAccessTokenView {
    return {
      id: t.id,
      name: t.name,
      prefix: t.prefix,
