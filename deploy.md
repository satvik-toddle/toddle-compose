...
- **Build Command:**
  ```
  corepack enable && pnpm install --filter backend... --frozen-lockfile && pnpm --filter @app/database generate && pnpm --filter backend build
  ```
...
- **Build Command:**
  ```
  corepack enable && pnpm install --filter rtc-server... --frozen-lockfile && pnpm --filter @app/rtc-database generate && pnpm --filter rtc-server build
  ```
