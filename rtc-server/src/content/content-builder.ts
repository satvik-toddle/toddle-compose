...
} from "../persistence/lexical-binding.core";
import {
  $createParagraphNode,
  ...
  return buildUpdate(baseState, (editor) => {
    editor.update(
      () => {
        ...
        root.clear();
        // Root accepts only ElementNodes/DecoratorNodes. $generateNodesFromDOM can still
        // yield top-level inline nodes (TextNode, LineBreakNode, inline decorators) for some
        // Coda HTML; collect consecutive inlines into one paragraph rather than appending a
        // bare inline (which throws "rootNode.splice: Only element or decorator nodes …").
        let inlineRun: LexicalNode[] = [];
        const flushInline = (): void => {
          if (inlineRun.length === 0) return;
          const p = $createParagraphNode();
          for (const n of inlineRun) p.append(n);
          root.append(p);
          inlineRun = [];
        };
...
// $generateNodesFromDOM's importDOM handlers call document.createElement/getComputedStyle etc.;
// install jsdom globals once per worker (a per-request worker pollutes only its throwaway globalThis).
...
// Parse an HTML string into a jsdom DOM document. The body-fragment is wrapped in a document skeleton
// so a leading block tag stays inside <body>; body-level <meta>/wrapper tags with no Lexical
// conversion are simply ignored by $generateNodesFromDOM.
