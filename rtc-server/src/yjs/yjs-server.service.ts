...
// Matches the `/yjs/<docId>` (collab) and `/locks/<docId>` (data-grid cell locks)
// endpoints. The token is accepted either as `?token=` (existing clients) or —
// when the query param is absent — as a WS subprotocol entry of the form
// `bearer.<token>` (lets browser clients avoid putting tokens in URLs). Clients
// using bearer-protocol auth must also offer the `yjs` protocol, since
