...
(async () => {
  ...
  const fieldInput = page.locator('.gs-field input[placeholder]');
  await fieldInput.waitFor({ state: 'visible', timeout: 5000 }).catch(() => {});
  pass('1b ⌘/Ctrl+K opens search modal', await page.locator('.gs-search').count() > 0);

  // Check 6: empty state = minimal prompt, no search request yet
  const emptyShown = await page.locator('.gs-empty').count() > 0;
  ...
  await page.locator('.sr').first().waitFor({ timeout: 5000 }).catch(() => {});
  const rowCount = await page.locator('.sr').count();
  const hasTitleBadge = await page.locator('.sr-badge').count() > 0;
  const hasHitMark = await page.locator('.sr .sr-hit').count() > 0;
  ...
  const isSplit = await page.locator('.gs-search.gs-split').count() > 0;
  const hasPreview = await page.locator('.gs-prev').count() > 0;
  ...
  const toggle = page.locator('.gs-field').getByRole('switch');
  const toggleCount = await toggle.count();
  if (toggleCount > 0) {
    await toggle.first().click({ force: true });
    await page.waitForTimeout(800);
    const nowNarrow = await page.locator('.gs-search.gs-sp').count() > 0;
    const previewGone = await page.locator('.gs-prev').count() === 0;
    ...
    pass('5-OFF toggle control present', false, 'no switch found in .gs-field');
  }
  ...
  await page.locator('.gs-field input[placeholder]').type('page');
  await page.waitForTimeout(1000);
  const stayedNarrow = await page.locator('.gs-search.gs-sp').count() > 0 && await page.locator('.gs-prev').count() === 0;
  ...
  const t2 = page.locator('.gs-field').getByRole('switch');
  ...
  await page.keyboard.press(process.platform === 'darwin' ? 'Meta+k' : 'Control+k');
  await page.locator('.gs-field input[placeholder]').waitFor({ timeout: 4000 }).catch(() => {});
  await page.locator('.gs-field input[placeholder]').type('pending');
  await page.waitForTimeout(1200);
  const sheetRowText = (await page.locator('.sr').allInnerTexts()).join(' | ');
  ...
  await page.locator('.gs-field input[placeholder]').type('page');
  await page.waitForTimeout(1000);
  await page.locator('.sr').first().click();
  await page.waitForTimeout(800);
  const navigated = /[?&]doc=/.test(page.url());
  const modalClosed = await page.locator('.gs-search').count() === 0;
  ...
  if (launcherPill) {
    ...
    await page.waitForTimeout(1200);
    const spotlight = await page.locator('.gs-search.gs-sp').count() > 0;
    const rowsG = await page.locator('.sr').count();
    // distinct workspace names shown in path lines
    const wsNames = new Set(await page.locator('.sr .sr-path').allInnerTexts());
